...
## Current Phase
**Phase 1D (Groceries) is COMPLETE and shipped to prod (Session 28, 2026-07-21) — 4 of 6 R1 phases done.** **BUG-004 (grocery-list latency) is now CLOSED + shipped (Session 30, 2026-07-21):** the ~27–37s batched `ingredient-normalize` is entirely off the confirm path — recipes normalize per-recipe during plan review (decoupled `plan.normalizeSlot` fired by the walker) and cache on the recipe row (`normalized_ingredients`, migration `0005`); confirm reads the cache + AI-normalizes only misses → zero AI calls at confirm on a fully-reviewed week. Phase D added the honest "Finishing N recipes…" straggler hint + early-confirm instrumentation. The real-model eval PASSED (per-recipe == batch merge quality; scallion↔green-onion synonym holds with no co-occurrence advantage). Code review found + fixed a high-sev rate-limit fan-out (`bgAiProcedure` — background normalize gets its own bucket so it can't 429 a user-visible hydrate). 306 unit + 53 E2E green. **BUG-002 (duplicate merge rows) + BUG-001 (quick-add category misfire) are now CLOSED + shipped (Session 31, 2026-07-22):** a **buy-unit table** in the aggregator (`src/server/grocery/buy-units.ts`) consolidates items a shopper buys as one thing — staples (salt/pepper/oils/spices) collapse to one unquantified row, concrete buy-unit produce (carrot→lb, onion→count) sums the buy-unit and absorbs off-units without fake conversion, keyed on canonicalName so it can never mis-merge; `guessCategory` now matches whole words. Both 300-line splits done (`aggregate.ts`→`quantity-parse.ts`, `grocery-generate.ts`→`grocery-collect.ts`). 327 unit + 53 E2E green. **Phase 1E (You tab) audit surface is BUILT + verified (Session 33).** Features #1 (shell + account), #2 (hard-constraint direct edit), #3 (memory ledger + `memory.deactivate`/`reactivate`), #5 (capture confirmation + undo), #6 (implicit surfaced/dismissible) shipped against the imported Direction A, plus the design's hero — the **AI capture task `user.talk`** (Griffin chose to build it now): `preferences-talk` prompt (SAFETY block, snapshot-tested) + `[N]`-ref id-safety + pure `applyPreferencesTalkOps`. Verified: **363 unit + 62 E2E** (first You coverage, Y1–Y9), **real-model safety eval 9/9 (0 safety failures)**, visual-QA (0 blockers/0 high vs Direction A), code review (no critical; 4 findings fixed). Allergy weighting is a `(allergy)` string marker (no schema change), stripped in `getChefContext`. **#4 (onboarding interview) is BUILT (Session 36)** — household-composition schema (band counts + a baby-stage follow-up; `householdSize` derived server-side; migration `0006`), a deterministic tunable deep-round **stopping policy** + 6-persona eval (`scripts/1e-onboarding-planner-eval.ts`), the full one-model-per-screen flow at `/welcome` (`src/components/onboarding/*`, `src/lib/onboarding/*`), the first-run gate on `users.onboardingCompletedAt` (read via `ensureOnboarded`, set by BOTH complete and skip), `sourceType:'onboarding'` memory writes, and the pre-seeded Plan intent hand-off (door #3). **437 unit green, planner eval 6/6, lint+typecheck clean.** **1E is code-complete but NOT closed:** E2E specs OB1–OB8 are written and UNRUN, blocked by **BUG-007** (Supabase rejects the `service_role` key with `bad_jwt`/ES256, so the whole harness can't authenticate) — `/visual-qa`, `/code-review`, and Griffin's taste pass all wait on that key refresh. **New interstitial Phase 1E.5 (Plan Design Buildout) formalized S34** — a full all-states Plan-tab rebuild in Claude Design → code, which **must ship before 1F** (Plan is the only core surface never mocked in the Claude Design system-of-record; 1F polishes on top of it, doesn't re-design it). Brief comes after the interview locks. See `docs/scope-v1.md`. Open bugs: BUG-003 (recipe.list harvest), BUG-005 (app-wide `font-sans`→serif, confirm on-device, 1F). Work is on a branch pending Griffin's taste pass before merge-to-prod. See `docs/whats-next.md` + `docs/scope-1E.md`.** — History below is retained for context.
