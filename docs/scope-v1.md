...
- [ ] Ship R1 as an installable PWA (manifest, service worker, offline shell, home-screen icon set, ~~install prompt~~) — validated on Griffin's + wife's phones. Rides with the design pass; native mobile stays held (decision 2026-07-24, see decisions.md). **Code-complete as of S61** — manifest + icon pipeline (S59), service worker + offline shell + queued check-off (S60), and the three design artifacts (S61: the Ember icon, the Hero launch screen, the offline clause). ⛔ **The install prompt was CUT by the design round** (S60): two users, both told how to add it by hand, so an in-app prompt would spend the product's first act teaching a browser gesture the app is not allowed to perform. **The only thing left in C is the two-phone check**, which is the item most likely to be quietly wrong on a real device
...
- [x] Observability: PostHog + Sentry — **done S63.** ⚠️ **The "event taxonomy from S9" this line cited for
      44 sessions did not exist** (the plan holding it is recorded LOST in `plans/README.md` since S18), so
      it was **designed**: 34 events / 8 categories in [observability-taxonomy.md](observability-taxonomy.md),
      typed so a wrong name — or any property that could carry user content — is a compile error. The whole
      north-star funnel is wired, so **time-to-list is now a query** and **Workstream E is unblocked**.
      ⚠️ Owed: look at one real session recording (needs Griffin's PostHog project); 26 of 34 events are
      declared but not yet wired, tracked by `wiring.test.ts`
...
1. ~~Does 1F include a small closed beta beyond Griffin + wife, or is two-user validation enough to ship
   ...
   - **What it actually removes from 1F:** a support path, an onboarding-for-strangers pass, and
     multi-user load/abuse/cost modelling. It removes no build work on the gate itself, which is the part
     that looked expensive and was already done.
   - **⚠️ CORRECTED S60 — this list used to open with "in-app feedback capture," and that was a bundling
     error.** Feedback capture is **back in R1 as 1F Workstream E** (Griffin, S60). Every other item here
     is justified by *there are no strangers*; feedback capture is not, because **its primary user is
     Griffin testing on his couch**, and that justification survives "no closed beta" untouched. It was
     cut only because it shared a sentence with the support path. **The generalisable check: does every
     item on a decision's fallout list fail for the same reason the decision gives?** This one did not,
     and it then propagated verbatim into three docs for eleven sessions.
   - **It does not lower the validation bar.** The DoD still requires Griffin **and his wife** to each run
     the full ritual on prod for 2 consecutive real weeks.
   - **⚠️ FACTUAL CORRECTION, S60.** This bullet used to continue *"and she is the nearest thing R1 has to
     a cold user, since she has been in none of these sessions."* **False on both counts.** She is a
     **software engineer**, she has **sat beside Griffin for much of this build**, and she will **test as
     aggressively as he does** — a second tester with context, not a stranger-proxy. **R1 has no cold user
     and should stop claiming one.** The claim was written once in S49, repeated verbatim here and in
     `decisions.md`, and by S60 it was quietly load-bearing under three recommendations about her account
     and her feedback. All three collapsed as soon as it was checked.
...
| Date | Change | Why |
|------|--------|-----|
| 2026-08-03 (S63) | **Observability shipped — 1F/D's biggest remaining item, and the gate on Workstream E.** 34-event taxonomy ([observability-taxonomy.md](observability-taxonomy.md)), PostHog + Sentry, session replay with the masking posture inverted, and the **complete north-star funnel wired**, so the DoD's *"time-to-list measured < 10 minutes"* is now a query rather than a feeling. **BUG-054 🟠 CLOSED** (Griffin's call) and **`ALLOWED_EMAILS` decided YES** (his action, one env var). **811 unit green; E2E 141 passed / 1 failed** (`X6` — **BUG-058**, a real remount race in the Plan intent field, not a flake). Still owed in D: perf + a11y, the error-state sweep, prompt-injection review, secrets audit. | ⚠️ **The taxonomy this line cited for 44 sessions did not exist, and that is the finding.** Six documents named *"the event taxonomy from S9"* as an available input; its only source was one changelog line, and the artifact lived in a plan file `plans/README.md` has recorded as **LOST since S18** — never committed, deleted, unrecoverable. The plan's phase-skeleton half was consciously rescued into this doc at S19; its taxonomy half was not. **Fifth instance of the pattern and the sharpest variant: a claim about an ARTIFACT that another file in the same repo already recorded as destroyed**, four files apart the whole time. The same S9 row's *"vendor abstraction layer built in Phase 1"* was equally hollow — the file existed, with zero call sites, which is exactly what let it survive a reading. **The check: when a doc cites an artifact, open the artifact, not the sentence citing it.** Three further plan-vs-build gaps surfaced during the build itself: posthog-js has **no unmask API** (so the specified posture was only reachable via `maskTextFn`), the obvious `nav, button` allow-list would have **recorded the entire grocery list** (`grocery-row.tsx` draws the item name as a `<button>`), and Sentry's `tunnelRoute` would have been the S59 manifest bug a **third** time |
| 2026-08-03 (S62) | **BUG-053 🔴 CLOSED — the Groceries visual gate can see again**, and the full capture suite is **6 files / 55 states / all `ok`** across every surface. **Workstream D opened and is part-done:** migration safety ✅ (expand/contract in the drizzle rule, `migrations.test.ts`, `pg_dump` discipline), BUG-044 ✅, the security review part-done (authz/IDOR, rate limiting, SSRF, the access gates, the offline cache). **Observability is not started and it gates Workstream E.** **775 unit green.** New: **BUG-054** (Groceries has no heading in its non-ready states), **BUG-055** (`clearOfflineState()` misses the revocation paths), and an `ALLOWED_EMAILS` recommendation → `open-questions.md`. | ⚠️ **Two documented claims turned out to be false, and both had been repeated rather than measured.** BUG-053's filed prime suspect was the service worker; navigations are network-first, so it was never serving stale HTML — **the seed reset the SERVER and nothing had ever reset the CLIENT**, and `staleTime: 30_000` on a persisted cache explained every symptom at once. And Workstream D's own named security subject — *"the shell HTML is SERVER-RENDERED with the household's real content baked in"* — is **byte-for-byte untrue** (identical MD5s between an empty household and a seeded one). Written once in S60, copied into three docs, load-bearing under a security review nobody had checked it against: **fourth instance**, after the cold-user premise, the feedback-capture bundling, and §09's stale sentence. |
| 2026-08-02 (S61) | **1F/C is CODE-COMPLETE — the three locked design artifacts are built.** The Ember icon (replacing the S59 placeholder), the Hero launch screen (11 device PNGs + `apple-touch-startup-image` tags from one shared table), and the offline clause (`· offline` / `· sending` on the Groceries count, with the chef launcher as the only provisional control). New OF5/OF6/OF7 and a `grocery-offline-clause` capture state. **758 unit green.** **BUG-051 🟠 filed, routed to D.** The only thing left in C is **the two-phone check**. | ⚠️ **Every real defect was a CSS value transcribed into SVG by eye rather than by its definition** — an implicit `farthest-corner` extent, a blur radius that is twice the Gaussian σ, and an outer shadow clipped to outside its border box. None look wrong in a thumbnail; all three were caught by **sampling the rendered pixels against the arithmetic**. ⚠️ **And the locked artifact beat the reasoning that predated it:** the placeholder's toque carried a comment arguing for a size the artifact contradicts — *a comment explaining why a number is right is not evidence that it still is.* Two apparatus findings came with it: a true crossfade puts **two contradictory words in `textContent` at once** (corrected to a fade-swap), and **OF6 could not see a per-row badge**, the exact treatment its own subject forbids — *ask what the layer cannot see*, applied to a layer written the same session. |
