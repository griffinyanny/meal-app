...
| Modify feedback: immediate click ack + card-scoped acknowledgment | Session 12 (2026-05-29) | Two-part UX issue surfaced in Test 2 (chip-click modify). (1) **No immediate feedback on tap** — the chip looks idle until the mutation completes 2-5s later, breaking the click-to-action expectation. Minimum fix: dim/spinner the tapped chip + the card it belongs to immediately. (2) **Acknowledgment is page-scoped, not card-scoped** — the "Swapped X for Y" toast pops at the top of the page, where the user usually isn't looking; the change happened to one card, so the ack should live with the card. Options to design: bottom-anchored iOS toast, popover/badge attached to the card, brief green highlight on the updated card. Applies to ALL modify paths (card chip, expanded-sheet action, Talk-to-Chef free-form, future swap), not just chips. Hand to UX before implementing. |
