...
| Idea | Source | Notes |
|------|--------|-------|
| Dev-only debug HUD (copyable state blob, hotkey-toggled) | Session 16 (2026-07-08) | Griffin, during the manual QA pass: a dev-only overlay showing key app state he can copy-paste to Claude (kills the "describe your state in prose" round-trip; also lets Claude debug bug reports from a pasted blob). **Scale via a shared `DebugContext`**, not a per-tab panel: each surface registers state via one `useDebugPanel('plan', () => ({...}))` call; a `DebugHud` mounted in the app shell aggregates + dumps JSON with a Copy button, toggled by a hotkey. Dev-only (`NODE_ENV==='development'`, stripped from prod). Plan tab emits: derivedState (review/midweek/elapsed/streaming/intent), planId, status, slots[{date,timeframe,slotType,title}], pending, changedDates, ack/error. **Owner: the E2E harness thread** (reuses its seed/auth infra) — build as its Phase 0. Small. |
...
| Meal chips read as bare adjectives, not tappable actions | Session 15 (2026-07-06) | Test 7: expanded-sheet action chips showed "quick" / "high-protein" instead of imperative actions. Root cause TBD — those chips were stale AI output from the May plan (re-dated for testing), not a fresh generation. The prompt already specifies the right format ([chef-system.ts:104](../src/server/ai/prompts/chef-system.ts): "Exactly 2 short contextual modification chips … e.g. 'Make it spicier', 'Swap the protein', 'One-pan version'") and tests encode it. So either that old generation fumbled, or gpt-4.1-mini drifts to adjectives/tags despite the prompt. **Check against a fresh generation** (Session 15 deleted the stale plan for exactly this). If fresh chips are still weak: harden chef-system.ts line 104 ("imperative actions the user can tap, NOT adjectives or tags") as a DELIBERATE prompt change + update the snapshot test. Note the chips render as full-width action buttons in the expanded sheet, so a bare adjective reads especially badly there. Pairs with "tune recipe generation quality." |
