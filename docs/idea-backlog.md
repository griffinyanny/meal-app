...
| **[BUG-004 follow-up] Split `grocery-generate.ts` (313 > 300-line rule)** | Session 30 (2026-07-21) | The Phase C/D additions pushed `grocery-generate.ts` to 313 lines, over the CLAUDE.md 300-line non-test limit. Deliberately NOT split at ship time (churn risk right before deploy). Extract the collect/sweep/normalize helpers (`collectSourcedLines` + `sweepStragglers` + `soloFallback`) into a `grocery-collect.ts` sibling, leaving `generateGroceryList` as the orchestration. Low urgency, do on the next Groceries touch. Found by the S30 Phase-D code review. |
