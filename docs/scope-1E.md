...
**Phase status:** 🔨 Audit surface **BUILT + verified (S33)** — features #1/#2/#3/#5/#6 + AI capture (`user.talk`) + first You E2E; real-model safety eval + visual-QA + code review passed. **Only #4 (onboarding interview) remains — still design-gated** (Pass-2 not yet designed). 1E closes when #4 ships.
...
| 4 | **AI-guided onboarding interview (conversational, first-run)** | On first login a short chef-led conversation (not a form) seeds prefs + memories, then hands off to the first plan. **Skippable** ("skip for now" → sensible defaults, chef learns as you go). Completing it writes `user_preferences` + ≥1 `sourceType:'onboarding'` memory and sets an onboarding-complete flag (small additive schema). The first generated plan reflects the answers. | ⬜ **Design-gated** — Pass-2 not yet designed. The new-user state built in S33 is the *audit* sparse state, NOT this conversational flow. |
...
1. **Onboarding depth:** how many questions before it feels like a form? (Design Pass 2 decides the exact set; target ≤4 exchanges + skip.)
