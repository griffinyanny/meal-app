...
| RC16 | RECIPES_LIBRARY | `+` → Generate → type into the §09 control → its own send | **B7 / spec §09.** The generate dialog commits from the control's send; the full-width `Generate recipe` button is gone (it was a second filled cream button in the same viewport — §08 law 06), and the mic is visible at rest because §09 admits no text-only version. ⚠️ **Asserted on the REQUEST, not the response**: `buildRecipeFixture` falls back to a fixed title for a user-initiated generate, so a title assertion would pass against a build that dropped the typed text — S46's prompt-blind fixture exactly | 🟢 |
| RC17 | RECIPES_LIBRARY | Open the salmon → `Modify recipe` → type → send | **B7 / spec §09.** Same shape for the modify dialog. The title comes back `Miso-Glazed Salmon (Make it dairy-free)`, so one assertion proves BOTH the original recipe and the user's request reached the model. ⚠️ **This surface could not have been covered before S56** — the E2E mock had no `recipe-modify` fixture and throws on an unknown task, so the gap was a hole in the seam rather than a missing spec | 🟢 |
...
| SH2 | CONFIRMED → RECIPES_LIBRARY → *(detail)* → GROCERY_READY → YOU_RETURNING | Sweep every tab, then open the recipe detail | **B3 / spec §12 item 05.** Every icon-only control (`innerText` empty + an `<svg>` inside, derived from the rendered tree rather than a class list) measures ≥ **44×44**. Also asserts the sweep found *something*, because a broken selector would otherwise read as a clean app. ⚠️ **It opens the recipe DETAIL on purpose** — three of the app's icon buttons live only in that dialog, and the tab-only version of this sweep was structurally blind to them. ⚠️ **A SECOND blind spot in the same sweep, found in S56:** its Groceries and You legs waited on `nav` — the tab bar, which renders instantly on every route — while its Plan and Recipes legs waited on real content. Both of those tabs render a loading body until their tRPC query lands, so the sweep was measuring a **skeleton** on two of its five legs and reporting a clean app. Now waits on `grocery-list` and `you-safety-card`. **Writing the instrument does not exempt it from the question — and neither does fixing it once** | 🟢 |
...
| OF5 | GROCERY_READY | Go offline | Offline is announced **once**, as `· offline` on the count, with no fill and no border (measured, so law 05 never applies). ⚠️ **Most of this spec asserts ABSENCES** — no banner, no strip, no per-row badge, no second sentence, and the word appears exactly once in the whole list. Asserting only that the clause is present would pass just as happily against a build that also grew the banner the design round cut | 🟢 |
| OF6 | GROCERY_READY | Go offline, tick, reconnect | **The tick is pixel-identical offline** — background, border, radius and box measured against the online values, because a class-name check would pass on a build that dimmed it. *The queue is a fact about the app, not about the onion.* The chef launcher is the **only** control that changes, going provisional under its ordinary label | 🟢 |
| OF7 | GROCERY_READY | Tick offline, reconnect with the flush **slowed by a route handler** | One crossfade to `· sending`, then **the resolution is an absence** — nothing confirms, nothing lands, nothing needs dismissing. ⚠️ The delay is an INPUT, not a tolerance: on a local server a resumed mutation can land inside one React batch, so the word would never be set and the spec would go red for a reason that is not the bug (S57's *fix a race by construction*) | 🟢 |
...
| L6 | Browse is **honest** named doors **with counts**, and they **push**: through one, the tiles are gone and the view carries its own heading. S48: zero-count doors and doors identical to `All` are suppressed — the five-recipe seed shows three, and `Recently saved` is asserted absent. S55: the doors say `All` / `Cooked`, the Recipes tab's own words | `PICKABLE` | 🟢 |
...
| L18 | **A door's rows carry the door's premise** (S55): inside `Cooked`, the salmon row reads `Cooked May 9`, and the never-cooked half is asserted first so the spec cannot pass against a build that stamps a date on every row. The cooked clause used to render only in the negative, so in the one door where every row has a stamp it vanished and each row showed the month it was **saved** instead. ⚠️ Asserts the **fitting** row on purpose — `unfittableReason` REPLACES the meta, so the 40-minute carbonara on a 30-minute night carries no date at all, which is a considered trade rather than a defect. **Verified failing against pre-fix code** (physical file backup + full rebuild): the red read `"Miso-Glazed SalmonSaved in July · 25 min"`, the defect verbatim | `PICKABLE` | 🟢 |
| L19 | **The search field counts the room it is standing in** (S56, BUG-047): on the opening tier the placeholder carries the library count, and inside the pushed `Cooked` door it carries that door's own. ⚠️ **Both sides are read from the rendered DOM** — the advertised number against the actual `picker-row` count — because a hand-fed number would make the spec assert nothing (S55). It also asserts the two numbers **differ**, or it would pass against the bug it exists to catch. The placeholder used to be built from the `everything` tile regardless of the pushed door, so inside `Cooked` it promised to search 5 and searched 2 | `PICKABLE` | 🟢 |
