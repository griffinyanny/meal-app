...
**Phase status:** 🔨 Slice 0 ✅ + A ✅ + B ✅ + C ✅ + **Slice D partial (S26): staples #12 ✅ + Talk-to-Chef #11 ✅** → Recipes-tab reorg #14 pending its design pass (in flight), then Slice 5 wrap
...
| 14 | **Recipes-tab organization** | Cooked / deliberate-library / plan-drafts tiers derived from `isFavorite` + `lastCookedAt` + slot dates (no explicit "I cooked it"); search reaches everything; favoriting = promote; drafts clean up with their plan. Tight-budget fallback: fold cooked to top of "Your recipes" with a badge. | ⬜ (design pass in flight, S26 — brief at `docs/design/surfaces/recipes/brief.md`; **cooked signal resolved**: auto-stamp `lastCookedAt` from a confirmed plan slot whose date has passed. Build after the design lands, together with the harvest.) |
